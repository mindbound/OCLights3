...
import opengpu.ClientDrawThread;
import opengpu.Config;
...
public class PacketHandlerIMPL {
				...
				for (int i = 0; i < len; i++) {
					...
					if (!doThreadding)
						try {
							...
							OpenGPU.debug("failed to process command in clientsidedrawlist");
						}
					else {
						if (!thread.isAlive()) {
							OpenGPU.logger.log(Level.FATAL, "The client draw thread died, restarting");
							thread = new ClientDrawThread();
							thread.setName("OCLights2 Draw Thread");
							thread.start();
						}
	...
	public void clientLoggedIn(INetHandler clientHandler, NetworkManager manager) {
		if (Minecraft.getMinecraft().isSingleplayer()) {
			OpenGPU.debug("Singleplayer detected, sync not needed");
		} else {
			Config.setDefaults();
			OpenGPU.debug("PREP'd for SYNC");
		}
	}
